...
export function MentorshipPanel({ teacher, mentorships }: { teacher: Teacher; mentorships: Mentorship[] }) {
  ...
  return <section className="mentorship-panel"><div className="page-actions"><Button onClick={() => setFormOpen(true)}>新增指导学生</Button></div>{records.length ? <ul className="mentorship-list">{records.map((item) => <li key={item.id}><strong>{item.studentName}</strong><span>{item.academicYear} · {item.grade} · {item.major}</span><span>{item.topic} · {statusLabel[item.status]}</span></li>)}</ul> : <p>暂无科研指导记录。</p>}<Dialog open={formOpen} onClose={() => setFormOpen(false)} title="新增指导学生"><MentorshipForm teacherId={teacher.id} onSaved={() => setFormOpen(false)} /></Dialog></section>;
}
