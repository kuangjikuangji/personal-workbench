...
export function useUpdateStudentRecord() { const repositories = useRepositories(); const invalidate = useInvalidateStudents(); return useMutation({ mutationFn: ({ id, patch }: { id: string; patch: Partial<Omit<StudentRecord, 'id' | 'createdAt'>> }) => repositories.studentRecords.patch(id, patch), onSuccess: invalidate }); }
export function useDeleteStudentRecord() { const repositories = useRepositories(); const invalidate = useInvalidateStudents(); return useMutation({ mutationFn: (id: string) => repositories.studentRecords.delete(id), onSuccess: invalidate }); }
