...
export function useUpdateRecord() { const repositories = useRepositories(); const invalidate = useInvalidateTeacherData(); return useMutation({ mutationFn: ({ id, patch }: { id: string; patch: Partial<Omit<TeacherRecord, 'id' | 'createdAt'>> }) => repositories.teacherRecords.patch(id, patch), onSuccess: invalidate }); }
export function useDeleteRecord() { const repositories = useRepositories(); const invalidate = useInvalidateTeacherData(); return useMutation({ mutationFn: (id: string) => repositories.teacherRecords.delete(id), onSuccess: invalidate }); }
...
export function useUpdateMentorship() { const repositories = useRepositories(); const invalidate = useInvalidateTeacherData(); return useMutation({ mutationFn: ({ id, patch }: { id: string; patch: Partial<Omit<Mentorship, 'id' | 'createdAt'>> }) => repositories.mentorships.patch(id, patch), onSuccess: invalidate }); }
export function useDeleteMentorship() { const repositories = useRepositories(); const invalidate = useInvalidateTeacherData(); return useMutation({ mutationFn: (id: string) => repositories.mentorships.delete(id), onSuccess: invalidate }); }
