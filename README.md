...
成功联网读取账号资料后，浏览器只按 Supabase 用户 ID 缓存恢复界面所需的
最小资料（用户名、角色、启用状态和首次改密状态）。离线刷新仅在本地仍有
同一用户的未过期 Supabase 会话，且缓存资料显示账号已启用并已完成首次改密
时继续；联网校验失败、用户不匹配、会话过期或缓存不安全时都不会使用回退。
...
业务镜像、待同步队列与离线身份资料。在仍显示“离线，N 项待同步”时
不要退出登录或清除站点数据。
